...
public class MemberServiceImpl implements IMemberService {
	...
	@Override
	public boolean updateMember(String memId, String[] roles) throws BizNotEffectedException {
		
		if(memId == null || roles == null) {
			System.out.println("권한 미체크");
			throw new BizNotEffectedException();
		}
		System.out.println("--여기부터 오류발생--");
		Map<String,String> role_name = memberDao.getRolename(roles);
		List<Map.Entry<String, String>> list = new ArrayList<>(role_name.entrySet());
		List<UserRoleVO> update_list = new ArrayList<UserRoleVO>();
		int i=0;
		for (Map.Entry<String, String> entry : list) {
            System.out.println("MemberServiceImpl:-->Key: " + entry.getKey() + ", Value: " + entry.getValue());
            update_list.add(new UserRoleVO(memId,entry.getKey(),entry.getValue()));
        }
		
		if(memberDao.updateList(update_list)==1) {
			return true;
		}else {
			return false;
		}
