...
<script>
	...
	function join() {
		...
		$.ajax({
			...
			,success : function(data) {
				...
				if (data) {
					...
					$("#memId").val("");
					alert("사용중인 아이디입니다. 다른 아이디를 사용해주세요");
				}
			},
			error : function() {
				alert("error");
		...
		if (!idCheck) { //비동기통신 이어서 얘가 먼저 실행되서 두번씩 눌러야 동작함
		...
		<form:form name="loginForm" method="post" modelAttribute="member">
			<div class="int-area">
				...
				<button type="button" id="check_id" name="check_id"
					onclick="fn_checkId()">ID 확인</button>
			</div>
			<div class="int-area">
			...
			<div class="int-area">
				<input type="password" id="memPassCheck" name="memPassCheck"
					value="" autocomplete="off" required="required"> <label
					for=memPassCheck>RECONFIRM PASSWORD</label>
			</div>
			...
			<div class="int-area">
				<input type="text" id="memAdd1" name="memAdd1" value=""
					autocomplete="off" required="required"> <label for=memAdd1>ADDRESS
					1</label>
			</div>
			<div class="int-area">
				<input type="text" id="memAdd2" name="memAdd2" value=""
					autocomplete="off" required="required"> <label for=memAdd2>ADDRESS
					2</label>
			</div>
			<div class="int-area">
